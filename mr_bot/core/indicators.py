...
# ────────────────────────────────────────────────────────────────────────────
# Variance Ratio（Lo-MacKinlay 1988）— 取代 R/S Hurst 做 crypto mean-reversion 篩選
#
# 用 log-return（stationary），對 non-stationary price series 更 robust。
# VR(q) = Var(q-period log-return) / (q × Var(1-period log-return))
#   VR ≈ 1.0  → random walk
#   VR < 1.0  → mean reverting（反趨勢）
#   VR > 1.0  → trending（動量）
# ────────────────────────────────────────────────────────────────────────────

def variance_ratio(prices: Sequence[float], q: int = 5) -> Optional[float]:
    """
    計算 Variance Ratio VR(q)。

    需要至少 q*4+2 個 price（保守地給 AR(1) + VR 足夠資料）。
    返回 None 如果資料唔夠。
    """
    arr = [p for p in prices if p > 0]
    min_len = max(q * 4 + 2, 22)
    if len(arr) < min_len:
        return None

    log_ret = np.diff(np.log(np.array(arr, dtype=float)))
    if len(log_ret) < q + 1:
        return None

    # 1-period variance
    var1 = float(np.var(log_ret, ddof=1))
    if var1 <= 0:
        return None

    # q-period variance（overlapping）
    q_rets = np.array([log_ret[i:i + q].sum() for i in range(len(log_ret) - q + 1)])
    if len(q_rets) < 4:
        return None
    varq = float(np.var(q_rets, ddof=1))
...
class RollingVR:
    """
    Rolling Variance Ratio，每隔 compute_every 個 bar 才重算（減輕 CPU）。
    返回 VR(q)，None 如果暖機不足。
    """

    def __init__(self, window: int = 300, q: int = 5, compute_every: int = 50) -> None:
        self._window = window
        self._q = q
    ...
    def update(self, price: float) -> Optional[float]:
        ...
        self._count += 1
        if self._count % self._compute_every == 0:
            self._last_vr = variance_ratio(list(self._prices), q=self._q)
        return self._last_vr
